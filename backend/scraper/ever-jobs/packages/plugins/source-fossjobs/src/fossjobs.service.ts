...
export class FossJobsService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.error(`FossJobs scrape error: ${this.describeError(err)}`);
      return new JobResponseDto([]);
    }
  }
  ...
  private matchesSearch(item: FossJobsRssItem, searchTerm: string): boolean {
    ...
    return this.tokenizeSearchTerm(searchTerm).some(
      (part) =>
        title.includes(part) || description.includes(part) || category.includes(part),
    );
  }
