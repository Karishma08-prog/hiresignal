...
import { getAllCampaignRuns, getAllCampaigns, getRunLogs } from "@/lib/api/campaigns";
...
export default function CampaignsPage() {
  ...
  const latestRunId = useMemo(() => {
    return [...(campaignsQuery.data ?? [])]
      .filter((campaign) => campaign.lastRunId)
      .sort((a, b) => b.updatedAt.localeCompare(a.updatedAt))[0]?.lastRunId;
  }, [campaignsQuery.data]);
  const runLogsQuery = useQuery({
    queryKey: ["run-logs", latestRunId],
    queryFn: () => getRunLogs(String(latestRunId), 6),
    enabled: Boolean(latestRunId),
    refetchInterval: (runsQuery.data ?? []).some((run) => isActiveRunStatus(run.status))
      ? 4_000
      : false,
  });
  ...
  const campaignRows = campaigns.map((campaign) => {
    ...
    return {
      ...
      objective: campaign.objectiveFilterConfig.objective ?? "No objective configured yet.",
      ...
      titles: campaign.titleFilterConfig.includeTitles ?? [],
    };
  });
  ...
  const titleFilterCount = campaigns.reduce(
    (total, campaign) => total + (campaign.titleFilterConfig.includeTitles?.length ?? 0),
    0,
  );
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {campaign.titles.length ? (
                          ...
                          <span className="text-sm text-black/60">
                            No title filters configured yet
                          </span>
                        )}
                      </div>
                    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-[1.8rem] border border-blue-100 bg-white p-6 shadow-[0_12px_28px_rgba(15,15,15,0.05)]">
              ...
              <div className="mt-4 space-y-3">
                {(runLogsQuery.data ?? []).slice(0, 5).map((log) => (
                  <div
                    key={log.id}
                    className="rounded-[1.1rem] border border-blue-100 bg-blue-50 p-3 text-sm"
                  >
                ...
                {!runLogsQuery.data?.length ? (
                  <p className="text-sm text-black/60">
                    No run logs yet. Trigger a campaign run from the Scraper page.
                  </p>
                ) : null}
              </div>
            </div>
